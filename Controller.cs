...
namespace UsbUirt
{
    ...
    /// </summary>
    public sealed class Controller : IDisposable ///libera os recursos não gerenciados
    {
        #region Fields
        ...
        static Controller()
        {
            try
            {
                if (UUIRTGetDrvInfo(out _drvVersion) == false) ///o driver não foi reconhecido
                {
                    Marshal.ThrowExceptionForHR(Marshal.GetHRForLastWin32Error()); ///declara que houve uma falha, lançando uma exceção
                }
            }
            catch (Exception ex)
            {
                var a = ex.ToString();
                throw new ApplicationException("Unable to read UsbUirt driver version", ex); ///então declara q não achou o driver
            }
        }
        ...
        public Controller()
        {
            _learnStates = new Hashtable(); ///provedor de código hash e comparador
            _hDrvHandle = OpenDriver();

            UUINFO uuInfo = GetVersion();  /// checa a versão do firmware para ver a compatibilidade
        ...
        public bool BlinkOnReceive ///lógica booleana utilizada para o recebimento do sinal IR
        {
            get
            {
                CheckDisposed(); /// verifica se o atributo foi descartado
                return (GetConfig() & UUIRTConfigBits.BlinkOnReceive) != 0; ///configura e diz que retornou o sinal IR no USB UIRT 
                ///se ele for diferente de 0

            }
            set
            {
                CheckDisposed();
                SetConfig(value, BlinkOnTransmit, GenerateLegacyCodesOnReceive); ///***Dúvida*** 
        ...
        public bool BlinkOnTransmit ///lógica booleana utilizada para a trasmissão do sinal IR
        {
            get
            {
                CheckDisposed(); ///verifica se o atributo foi descartado
                return (GetConfig() & UUIRTConfigBits.BlinkOnTransmit) != 0; /// configura e diz que transmitiu o sinal IR se 
                /// ele for diferente de 0
            }
            set
            {
                CheckDisposed();
                SetConfig(BlinkOnReceive, value, GenerateLegacyCodesOnReceive); /// ***Dúvida*** 
            }
        }
        ...
        public bool GenerateLegacyCodesOnReceive
        {
            ...
            {
                CheckDisposed();
                SetConfig(BlinkOnReceive, BlinkOnTransmit, value); /// o código gerado vem aparti do sinal Recebido e sinal Enviado
            }
        }
        #endregion
        ...
        {
            add ///acessar um evento
            {
                CheckDisposed();
                lock (_receivedSyncRoot)  ///serve para bloquear uma instruções e só liberar quando concluída
                {
                    if (null == _received) /// ***Dúvida***
                    {
                        myCb = new ReceiveCallback(ReceiveCallbackProc);
                        SetReceiveCallback(myCb);
                    }
            ...
            remove ///quando põe o add é necessário ter o removedor de evento
            {
                CheckDisposed();
        ...
        public event LearningEventHandler Learning
        {
            add
            {
                ...
                {
                    _learning += value;    ///adiciona o evento se o valor for maior que o atributo _learning     
                }
            }
            remove
            {
                ...
                lock (_learningSyncRoot)
                {
                    _learning -= value; ///remove o evento se o valor for menor que o atributo _learning
                }
            }
        }
        ...
        {
            CheckDisposed();
            Transmit(irCode, _defaultTransmitCodeFormat, _defaultRepeatCount, _defaultInactivityWaitTime); ///transmite o código IR
        }
        ...
        {
            CheckDisposed();
            if (null == irCode) /// se o código transmitido for nulo
            {
                throw new ArgumentNullException("irCode", "irCode cannot be null"); ///declara que o código não pode ser nulo
            }

            if (0 == irCode.Length) ///se for =0
            {
                throw new ArgumentException("irCode", "irCode cannot be empty"); ///declara q o código não pode ser vazio
            }

            if (repeatCount < 0) ///se for <0
            {
                throw new ArgumentOutOfRangeException("repeatCount", "repeatCount cannot be negative"); ///declara q n pode ser negativo o código
            }

            if (inactivityWaitTime < TimeSpan.Zero) /// o tempo de espera n pode ser menor q o TimeSpan
            {
                throw new ArgumentOutOfRangeException("inactivityWaitTime", "inactivityWaitTime cannot be less than TimeSpan.Zero");
            }

            using (ManualResetEvent evt = new ManualResetEvent(false)) /// se n houver exceção transmitir IR
            {
        ...
        /// </summary>
        /// <param name="irCode">The IR code to transmit.</param>
        public void TransmitAsync(string irCode) ///código do IR assíncrono
        {
            CheckDisposed();
            TransmitAsync(irCode, _defaultTransmitCodeFormat, _defaultRepeatCount, _defaultInactivityWaitTime, null);/// transmitido os dados
        }
        ...
        //{
        //    CheckDisposed();
        //    return Learn(_defaultLearnCodeFormat, _defaultLearnCodeModifier, TimeSpan.Zero); ///retorna o aprendizado
        //}
        ...
        {
            CheckDisposed();
            return Learn(_defaultLearnCodeFormat, _defaultLearnCodeModifier, 0, timeout);
        }
            ...
            uint forcedFrequency, TimeSpan timeout)
        {
            ...
            using (SyncLearnResults results = new SyncLearnResults()) ///resultados do sincronismo de aprendizado
            {
                this.LearnCompleted += new LearnCompletedEventHandler(ManagedWrapper_LearnCompleted);///o aprendizado está completo se for maior
                ///ou igual a LearnCompletedEventHandler
                
                try
                {
                    LearnAsync(codeFormat, learnCodeFormat, forcedFrequency, results); /// busca os parâmetros
        ...
        /// <param name="userState">An optional user state object that will be passed to the 
        /// Learning and LearnCompleted events and which can be used when calling LearnAsyncCancel().</param>
        public void LearnAsync(CodeFormat codeFormat, LearnCodeModifier learnCodeFormat, uint forcedFrequency, object userState)
        {
            CheckDisposed(); /// exceções
            ...
            LearnState learnState = new LearnState(codeFormat, learnCodeFormat, forcedFrequency, userState); //jogado num atributo os 
            //parâmetros em LearnState
            _learnStates[learnStatesKey] = learnState;




            if (false == ThreadPool.QueueUserWorkItem(new WaitCallback(DoLearn), learnState))//Coloca um método para execução, e especifica 
            //um objeto que contém os dados a serem usados
            //pelo método. O método executa quando um 
            //segmento de pool de segmento 
            //estiver disponível.
            {
                throw new ApplicationException("Unable to QueueUserWorkItem");
            }
